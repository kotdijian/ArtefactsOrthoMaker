...
v0.4.3 から、石器の6面図も土器と同様に **直方体を正面図から展開する関係**を基準にします。
...
隣接する図の対応辺が、直方体を展開したときの接続関係になるように表示します。

特に Top / Bottom は次の向きです。
...
したがって、Front の上側に配置される Top では **Front 側が下、Back 側が上**になります。Front の下側に配置される Bottom では **Front 側が上、Back 側が下**になります。

左右方向も展開関係に合わせ、Top / Bottom と Front の画面右方向はいずれも `+X` です。
