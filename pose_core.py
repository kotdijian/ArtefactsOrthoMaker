...
def load_mesh_asset(path: str | Path, input_unit: str) -> MeshAsset:
    ...
    if path.suffix.lower() not in (".obj", ".ply", ".glb"):
        raise ValueError("v0.1.3 supports OBJ, PLY and GLB only.")
...
def transformed_mesh(asset: MeshAsset, matrix: np.ndarray) -> trimesh.Trimesh:
    """Return a transformed copy while reusing normals computed at load time.

    Do not ask Trimesh to recompute weighted vertex normals here. On large meshes
    that path may use SciPy sparse matrices (or a much heavier fallback when SciPy
    is absent). The app already validates/calculates vertex normals at load time, so
    for our rigid pose transform we only need to rotate and renormalize them.
    """
    out = asset.mesh.copy()
    out.vertices = transform_points(np.asarray(asset.mesh.vertices), matrix)

    rot = np.asarray(matrix, dtype=float)[:3, :3]
    normals = np.asarray(asset.vertex_normals, dtype=float) @ rot.T
    lengths = np.linalg.norm(normals, axis=1)
    good = lengths > EPS
    normals[good] /= lengths[good, None]
    normals[~good] = np.array([0.0, 0.0, 1.0])

    # Vertex assignment invalidates caches. Populate the already-computed result
    # directly so OBJ/PLY exporters can include normals without invoking SciPy.
    try:
        out._cache.clear()
        out._cache["vertex_normals"] = normals
    except Exception:
        pass
    return out
